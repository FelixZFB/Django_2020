...
- 5.配置mysql数据库，在test3/settings.py中DATABASES项配置使用MySQL数据库
    ...
    - settings.py中DATABASES参数进行数据库配置,具体查看settings.py文件
    - 注意：启动django之前要启动mysql服务器
...
- 登陆案例详情查看：
    - booktest/views.py 中的login和login_check
    - 密码正确，跳转到首页
    - 密码不对，刷新页面
...
- ajax使用：
    ...
    ![ajax请求过程](99_Django笔记图片/033_ajax请求过程.png)
- ajax请求过程都是在后台完成，前端页面不会有任何变化
    - 具体过程进network中查看
    - 进具体ajax请求的response中查看详细信息

- ajax登陆案例
    - 查看templates/login_ajax.html和/booktest/views.py中的login_ajax及login_ajax_check函数
