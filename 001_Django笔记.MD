...
- 静态文件使用：
    ...
    - settings.py配置静态文件对应的URL地址和访问路径
        ...
        - 可以将默认的static修改为abc(相当于给静态文件夹重命名了)，此时模板html文件源码中就要使用abc，使用static就访问不到静态文件了
        - STATIC_URL = '/abc/'
...
- 后台管理图片需要安装Pillow
    - pip install Pillow
- 1.项目文件夹，一般是static下面创建media文件夹用于存放上传的图片，里面在创建具体应用对应的子文件夹
    - test4/static/media
    - test4/static/media/booktest
- 2.settings里面设置文件夹路径
    - MEDIA_ROOT = os.path.join(BASE_DIR, 'static/media') 
- 3.创建类，注册类
    - models.py里面创建图片管理类,字段使用图片类型，指定文件夹参数
    - admin.py里面自定义后台管理类然后注册类
- 4.生成迁移文件执行迁移命令
    - python manage.py makemigrations
    - python manage.py migrate
- 5.启动服务器进入后台管理
    - 此时已有图片类，然后进去添加图片
    - 图片就被上传到static/media/booktest文件夹下面
    - 后台管理显示的类容是图片的路径
    - ![后台上传图片](99_Django笔记图片/052_后台上传图片.png)
    
    
- 方式2：自定义页面上传图片
- 1.自定义上传图片html页面，查看test4/templates/booktest/pic_upload.html
- 2.定义视图函数和配置URL
    - 查看test4/booktests/views.py中的pic_upload和pic_handle函数
    - 查看test4/booktests/urls.py
- 3.HTML页面上传图片
- ![自定义上传图片](99_Django笔记图片/053_自定义上传图片.png)
...
## 8.5 迁移文件管理
- 迁移文件管理：
    - 生成迁移文件，应用booktest/migrations文件夹下面根据models.py生成迁移文件
        - 第一次执行时候，生成的迁移文件名为0001_initial.py
        - 后面添加了新的模型，生成的文件名是0002_模型类名称小写，迁移文件dependencies依赖是0001文件
        - 后面又添加了新的模型，生成的文件名是0003_模型类名称小写，迁移文件dependencies依赖是0002文件
    - 执行迁移命令，生成mysql里面对应的表格
        - 只会执行最新未执行过迁移命令的迁移文件 
        - mysql的项目数据库中有个 django_migrations 表
            - 表中就是对应的所有已经执行过迁移的文件
            - 可以看见booktest下面执行过的所有迁移文件
            - 如果要重新生成迁移文件，执行迁移命令，生成新的表就要删除迁移文件表中的文件和数据库项目下的对应表
            - 注意删除迁移文件，只能删除应用下文件，其它不要动，使用id删除
            - 比如：delete from django_migrations where id=17;
        - ![迁移文件管理说明](99_Django笔记图片/051_迁移文件管理说明.png)
...
- 具体查看：
    - test4/booktest/views.py 中show_area函数
    - test4/booktest/urls.py
    - test4/templates/booktest/show_area.html

- Paginator类实例对象
    - 实例对象：paginator = Paginator(areas, 5)  每页显示5条数据的实例对象
...
Page类实例对象
    - 实例对象：page = paginator.page(1)   第一页的实例对象
    - 调用Paginator实例对象即paginator的page()方法返回Page实例对象即page，不需要手动构造。
    - 属性object_list：返回当前页对象的列表。
    - 属性number：返回当前是第几页，从1开始。
    - 属性paginator：当前页对应的Paginator对象。
    - page.previous_page_number 当前页面的上一页是第几页，值是数字
    - page.next_page_number 当前页面的下一页是第几页，值是数字
    - 方法has_next()：如果有下一页返回True。
    - 方法has_previous()：如果有上一页返回True。
    - 方法len()：返回当前页面对象的个数。
    
## 8.7 多级选择(省市区选择)
- 在Django中使用jquery的ajax进行数据交互。 
    - jquery框架中提供了$.ajax、$.get、$.post方法，
    - 用于进行异步交互，由于Django中默认使用CSRF约束，推荐使用$.get。
    - ajax请求获取信息，使用get
    - ajax请求涉及到信息修改，使用post
    
- 省市区选择案例就是使用的ajax的get请求
    - 具体查看：
    - test4/booktest/views.py 中的prov函数和city函数
    - test4/booktest/urls.py
    - test4/templates/booktest/areas.html
    - 代码中有详细注释
    

# 9 部署
## 9.1 搭建服务器虚拟环境
- 1）在本机进入项目虚拟环境，执行命令导出当前需要的所有包。
    - 比如进入虚拟环境后，切换到项目文件夹根目录下，执行命令：
    - pip freeze > requirements.txt
    - requirements.txt 里面就是项目依赖的包

- 2）通过ftp软件将项目代码和requirements.txt文件上传到服务器。

- 3）服务器上，创建虚拟环境，在虚拟环境上安装包。
    - mkvirtualenv 虚拟环境名称
    - pip install -r requirements.txt

- GitHub项目拉取到不同操作系统，只保留项目本身内容，然后可以创建虚拟环境，进入虚拟环境后安装依赖包

## 9.2 uWSGI 和 Nginx
- uWSGI实现了WSGI的所有接口，是一个快速、自我修复、开发人员和系统管理员友好的服务器。
- uWSGI代码完全用C编写，效率高、性能稳定。
- Nginx: 使用nginx的作用主要包括负载均衡、反向代理。
- 具体查看html教程6.5节